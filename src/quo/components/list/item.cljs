...
(defn title-column
  [{:keys [title text-color subtitle subtitle-max-lines subtitle-left
     ...
     [:<>
      ...
      (if (string? title)
        [text/text {:weight              (or title-text-weight :medium)
         ...
         title]
        title)
      (if (string? subtitle-left)
        [rn/view {:flex-direction :row}
         [text/text {:style {:max-width "46.5%"}
                     :weight          :regular
                     :color           :secondary
                     :ellipsize-mode  :tail
...
(defn list-item
  [{:keys [theme accessory disabled subtitle-max-lines icon icon-container-style
           left-side-alignment
           title subtitle subtitle-left active on-press on-long-press chevron size text-size
